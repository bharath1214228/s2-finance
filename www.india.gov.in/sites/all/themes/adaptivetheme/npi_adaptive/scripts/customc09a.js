		...
		attach: function (context, settings) {
			$(document).ready(function () {
				...
				$('#queries a').each(function () {
					...
					if (jQuery(this).text() == "search - keyword, phrase" || jQuery(this).text() == "à¤–à¥‹à¤œà¥‡à¤‚ - à¤–à¥‹à¤œà¤¶à¤¬à¥à¤¦, à¤µà¤¾à¤•à¥à¤¯à¤¾à¤‚à¤¶") {
						jQuery(this).hide();
					} else {
						i++;
						if (i > 5) {
...
$(document).ready(function () {
	...
	$("#block-views-home-main-slider-block ul.flex-direction-nav .flex-prev").attr("title", "Previous");
	$("#block-views-home-main-slider-block ul.flex-direction-nav .flex-prev").attr("aria-label", "Previous");
	$("#block-views-home-main-slider-block .flex-pauseplay .flex-pause").attr("title", "pauseplay");
	$("#block-views-home-main-slider-block .flex-pauseplay .flex-pause").attr("aria-label", "pauseplay");
	$("#block-views-home-main-slider-block ul.flex-direction-nav .flex-next").attr("title", "Next");
	$("#block-views-home-main-slider-block ul.flex-direction-nav .flex-next").attr("aria-label", "Next");
	$("#block-views-home-main-slider-block .flex-pauseplay .flex-play").attr("title", "pauseplay");
	$("#block-views-home-main-slider-block .flex-pauseplay .flex-play").attr("aria-label", "pauseplay");
	...
	$("a[href^=http]").click(function (e) {
		...
		if (domain) {
			...
			if (domain != hostname) {
				...
				if(domain=='india.gov.in' || domain=='xn--i1bj3fqcyde.xn--11b7cb3a6a.xn--h2brj9c'){
					...
					if(id=='switch-lang'){
						...
						if(curr_lang=='English')
							var confirm_msg="This will lead you to " + lang_title + " language. Are you sure to switch to this language?";
						else if(curr_lang=='Hindi')
							var confirm_msg="यह आपको " + lang_title + " भाषा में ले जाएगा। क्या आप वाकई इस भाषा में स्विच करना चाहते हैं?";
						else
							confirm_msg="Are you sure to switch to this language?";
					...
					if(curr_lang=='English')
						var alert_msg="You are being redirected to an external website. Please note that National Portal is not responsible for external websites content & privacy policies.";
					else if(curr_lang=='Hindi')
						var alert_msg="आपको एक बाहरी वेबसाइट पर रीडायरेक्ट किया जा रहा है। कृपया ध्यान दें कि राष्ट्रीय पोर्टल बाहरी वेबसाइट सामग्री और गोपनीयता नीतियों के लिए ज़िम्मेदार नहीं है।";
					else
						alert_msg="You are being redirected to an external website. Please note that National Portal is not responsible for external websites content & privacy policies.";
...
$(document).ready(function () {
	...
	$('ol.result-page li').each(function() {
		$(this).find('.share_btn').focus(function(e) {
			$(this).parents('.share-icon').addClass('mFocus').siblings().removeClass('mFocus');
		});	
	});

	$("ol.result-page .share-icon .sharebtns li:last-child a").focusout(function(e) {
	...
	$(".quicktabs-tabs li a").each(function (index) {
		let viewId = $(this).parents(".quicktabs-wrapper").find(".quicktabs_main div.quicktabs-tabpage").eq(index).attr("id");
	...
	$(".quicktabs-tabs li a").on("keyup", function (event) {
		...
		if ( $tabAtags.length == 0 || !$($tabId).find(".block-content").is(":visible") ) {
	...
	$(".signup_formouter .description, #account-pass-restrictions").attr("tabindex", 0);
